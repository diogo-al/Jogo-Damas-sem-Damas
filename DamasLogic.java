...
public class DamasLogic {
	...
	void posFill() { //funcao para preencher o vetor POS
	...
	void firstPlace() { //funcao para colocar as pecas nos sitios certos quando se abre um jogo novo
	...
	boolean isWithinBounds(int line, int col) { // funcao para verificar se uma dada linha e coluna estao dentro dos limites do tabuleiro
		return((line >= 0 && line < length) && (col >=0 && col< length));
	}
	
	boolean isDraw() { // funcao para ver se o jogo esta empatado
	...
	boolean validPlay(int initialLine,int initialCol, int finalLine, int finalCol){ // funcao para ver se uma dada jogada e valida
		if(captureAvailable()){
	...
	void moveTo(int initialLine, int initialCol, int finalLine, int finalCol) { //funcao para mover uma peca
		if(isWhiteTurn) {
	...
	void oneColorPiece() { //funcao que preenche um vetor de positions com as posicoes das pecas de uma so cor
		int count = 0;
		if(isWhiteTurn) {
	...
	void randomPlay(){ // funcao que joga aleatoriamente
		oneColorPiece();
		Error1 = false;
		int count = 0;
		int numberOfTrys = 0;
	...
	boolean isPossibleTocapture(int initialLine, int initialCol,int finalLine,int finalCol) { // funcao que analisa se e possivel capturar numa dada jogada
	...
	boolean isBlocked() { //funcao para ver se e possivel jogar matas mais no futuro
		int count = 0;
		for(int l = 0; l < length ; l ++) {
			for(int c = 0; c<length; c++) {
	...
	boolean isPossibleToMove() { //funcao que ve se e possivel mover pelo menos uma peca
		for(int l = 0; l < length ; l ++) {
			for(int c = 0; c<length; c++) {
	...
	boolean captureAvailable() { //funcao que ve se existe alguma captura possivel
		for(int l = 0; l < length ; l ++) {
			for(int c = 0; c<length; c++) {
				if(("white").equals(pos[l * length + c].piece()) && isWhiteTurn || ("black").equals(pos[l * length + c].piece()) && !isWhiteTurn) {
	...
	void capture(int initialLine, int initialCol,int finalLine,int finalCol) { //funcao para capturar
		int middlePieceLine = (initialLine + finalLine)/2;
		int middlePieceCol = (initialCol + finalCol)/2;
	...
	boolean win() { //funcao para ver se ha vitoria e quem ganhou
	...
	void saveGame(String saveName) { //funcao apra salvar o jogo
		try {
