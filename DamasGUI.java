...
public class DamasGUI {
	private Board board;
	private DamasLogic logic;
	private int count = 0;
	private int initialLine = -1;
	...
	DamasGUI(DamasLogic newLogic, int length) {
		...
		if(logic.getWturn())
			...
			board = new Board("As Pretas jogam   " + logic.getNumberOfWStones() + "B | " + logic.getNumberOfBStones() + "P", length, length, 80);
		board.addMouseListener(this::click);
		board.setBackgroundProvider(this::background);
		board.setIconProvider(this::icon);
		board.addAction("Random", this::random);
